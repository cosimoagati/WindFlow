...
#if !defined (__SHARED__)
        Stateful_Processing_Kernel<<<num_blocks, warps_per_block*threads_per_warp, 0, records[id_r]->cudaStream>>>(b->data_gpu, records[id_r]->map_idxs_gpu, records[id_r]->dist_keys_gpu, records[id_r]->start_idxs_gpu, records[id_r]->state_ptrs_gpu, (b->kb).num_dist_keys, num_active_thread_per_warp);
#else
        Stateful_Processing_Kernel<<<num_blocks, warps_per_block*threads_per_warp, sizeof(Window_State) * num_active_thread_per_warp * warps_per_block, records[id_r]->cudaStream>>>(b->data_gpu, records[id_r]->map_idxs_gpu, records[id_r]->dist_keys_gpu, records[id_r]->start_idxs_gpu, records[id_r]->state_ptrs_gpu, (b->kb).num_dist_keys, num_active_thread_per_warp);   
#endif
        //Stateful_Processing_Kernel<<<num_blocks, warps_per_block*threads_per_warp, sizeof(Window_State) * num_active_thread_per_warp * warps_per_block, records[id_r]->cudaStream>>>(b->data_gpu, records[id_r]->map_idxs_gpu, records[id_r]->dist_keys_gpu, records[id_r]->start_idxs_gpu, records[id_r]->state_ptrs_gpu, (b->kb).num_dist_keys, num_active_thread_per_warp);
        batch_to_be_sent = b;
        id_r = (id_r + 1) % 2;
...
int main(int argc, char * argv[]) {
	...
	for (size_t i=0; i<map_degree; i++) {
		ff_comb *comb_node = new ff_comb(new dummy_mi(), new Map(i, map_degree, app_start_time, batch_size), true, true);
