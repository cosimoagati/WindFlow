...
public:
    ...
    void prepare_batch(tuple_t &t)
    {
        if (data_gpu[id_r] == nullptr) {
            ...
            b = new batch_t<tuple_t, size_t>(batch_size, data_gpu[id_r], data_gpu[id_r], recycle_queue, delete_counter);
        }
...
// CUDA Kernel Stateless_Processing_Kernel
__global__ void Stateless_Processing_Kernel(tuple_t *tuples,
											bool *flags,
                                            size_t len,
                                            Filter_Functor &_func,
...
public:
        ...
        filterF(_num_keys)
    {
        ...
        gpuErrChk(cudaMemcpyAsync(filterF_gpu, &filterF, sizeof(Filter_Functor), cudaMemcpyHostToDevice, cudaStream));
    }
    ...
    batch_t<tuple_t, size_t> *svc(batch_t<tuple_t, size_t> *b)
    {
        ...
        int num_blocks = std::min((int) ceil(((double) (b->size)) / warps_per_block), numSMs * max_blocks_per_sm);
		Stateless_Processing_Kernel<<<num_blocks, warps_per_block*threads_per_warp, 0, cudaStream>>>(b->data_gpu, flags_gpu, b->size, *filterF_gpu, num_active_thread_per_warp);
        ...
        auto end = thrust::copy_if(thrust::cuda::par(alloc).on(cudaStream), th_data_gpu, th_data_gpu + b->size, flags_gpu, th_new_data_gpu, pred);
        // change the logical size of the batch
        b->size = end - th_new_data_gpu;
        gpuErrChk(cudaMemcpyAsync(b->data_gpu, new_data_gpu, b->size * sizeof(tuple_t), cudaMemcpyDeviceToDevice, cudaStream));
...
int main(int argc, char * argv[]) {
	...
	for (size_t i=0; i<map_degree; i++) {
		ff_comb *comb_node = new ff_comb(new dummy_mi(), new Filter(i, map_degree, app_start_time, batch_size, num_keys), true, true);
