...
struct tuple_t {
    ...
    // Constructor I
    __host__ __device__ tuple_t(): property_value(0.0),
    ...
    // Constructor II
    __host__ __device__  tuple_t(double _property_value,
...
struct batch_t
{
    ...
    struct keyby_record // records of fields used for the keyby distribution
    {
	    ...
	    atomic<size_t> *ready_counter; // pointer to the atomic counter to be used to complete the keyby processing
    };
    ...
    batch_t(size_t _size,
    	    in_t *_data_gpu,
    	    in_t *_raw_data_gpu,
    	    MPMC_Ptr_Queue *_queue,
    	    atomic<size_t> *_delete_counter,
    	    atomic<size_t> *_ready_counter=nullptr):
            size(_size),
            data_gpu(_data_gpu),
            raw_data_gpu(_raw_data_gpu),
            queue(_queue),
...
public:
    // Constructor
    Source(size_t _n_dest,
    	   vector<tuple_t> &_dataset,
           size_t _batch_size,
           ...
           recycle_queue(_recycle_queue)
    {
        ...
        previous_bouts = (batch_t<tuple_t, size_t> **) malloc(sizeof(batch_t<tuple_t, size_t> *) * n_dest);
    }
    ...
    void prepare_batch(tuple_t &t)
    {
        if (data_gpu[id_r] == nullptr) {
        	...
        	for (size_t i=0; i<n_dest; i++) {
        		bouts[i] = new batch_t<tuple_t, size_t>(batch_size, data_gpu[id_r], data_gpu[id_r], recycle_queue, delete_counter, ready_counter);
        	}
        }
        ...
        if (tuple_id == batch_size) {
            ...
            gpuErrChk(cudaMemcpyAsync(data_gpu[id_r], data_cpu[id_r], batch_size * sizeof(tuple_t), cudaMemcpyHostToDevice, cudaStreams[id_r]));
...
__device__ bool map_function(tuple_t &t, Window_State &state)
{
...
__global__ void Stateful_Processing_Kernel(tuple_t *tuples,
										   bool *flags,
                                           int *map_idxs,
                                           size_t *dist_keys,
                                           int *start_idxs,
                                           ...
                                           int num_active_thread_per_warp)
{
    ...
    if (id % threads_per_worker == 0) {
        for (int id_key=id_worker; id_key<num_dist_keys; id_key+=num_workers) {
            ...
            while (idx != -1) {
                //tuples[idx].incremental_average = states[id_key]->compute(tuples[idx].property_value);
                flags[idx] = map_function(tuples[idx], *(states[id_key]));
                idx = map_idxs[idx];
            }
        }
    }
...
public:
    // Constructor
    Filter(size_t _id_map,
    	size_t _map_degree,
    	const unsigned long _app_start_time,
    	size_t _max_batch_len,
    	bool *_flags_gpu):
        id_map(_id_map),
        ...
        app_start_time(_app_start_time),
        current_time(_app_start_time),
        max_batch_len(_max_batch_len),
        flags_gpu(_flags_gpu)
    {
    ...
    batch_t<tuple_t, size_t> *svc(batch_t<tuple_t, size_t> *b)
    {
        ...
        if (num_new_keys > 0) {
        	...
        	int num_blocks = std::min((int) ceil(((double) num_new_keys) / threads_per_block), numSMs * max_blocks_per_sm);
        	Initialize_States_Kernel<<<num_blocks, threads_per_block, 0, records[id_r]->cudaStream>>>(records[id_r]->new_state_ptrs_cpu, num_new_keys);
        }
        gpuErrChk(cudaMemcpyAsync(records[id_r]->state_ptrs_gpu, records[id_r]->state_ptrs_cpu, (b->kb).num_dist_keys * sizeof(Window_State *), cudaMemcpyHostToDevice, records[id_r]->cudaStream));
		gpuErrChk(cudaMemcpyAsync(records[id_r]->dist_keys_gpu, (b->kb).dist_keys_cpu, (b->kb).num_dist_keys * sizeof(size_t), cudaMemcpyHostToDevice, records[id_r]->cudaStream));
		gpuErrChk(cudaMemcpyAsync(records[id_r]->start_idxs_gpu, (b->kb).start_idxs_cpu, (b->kb).num_dist_keys * sizeof(int), cudaMemcpyHostToDevice, records[id_r]->cudaStream));
		gpuErrChk(cudaMemcpyAsync(records[id_r]->map_idxs_gpu, (b->kb).map_idxs_cpu, b->size * sizeof(int), cudaMemcpyHostToDevice, records[id_r]->cudaStream));
        ...
        int num_blocks = std::min((int) ceil(((double) (b->kb).num_dist_keys) / warps_per_block), numSMs * max_blocks_per_sm);
        if (batch_to_be_sent != nullptr) {
            ...
            if (batch_to_be_sent->isKBDone()) {
		        ...
		        thrust::device_ptr<tuple_t> th_data_gpu = thrust::device_pointer_cast(batch_to_be_sent->data_gpu);
		        thrust::device_ptr<tuple_t> th_new_data_gpu = thrust::device_pointer_cast(new_data_gpu);
		        auto pred = [] __device__ (bool x) { return x; };
		        auto end = thrust::copy_if(thrust::cuda::par(alloc).on(records[(id_r + 1) % 2]->cudaStream), th_data_gpu, th_data_gpu + batch_to_be_sent->size, flags_gpu, th_new_data_gpu, pred);
		        // change the logical size of the batch
		        batch_to_be_sent->size = end - th_new_data_gpu;
		        gpuErrChk(cudaMemcpyAsync(batch_to_be_sent->data_gpu, new_data_gpu, batch_to_be_sent->size * sizeof(tuple_t), cudaMemcpyDeviceToDevice, records[(id_r + 1) % 2]->cudaStream));
		        gpuErrChk(cudaStreamSynchronize(records[(id_r + 1) % 2]->cudaStream));
...
#if !defined (__SHARED__)
        Stateful_Processing_Kernel<<<num_blocks, warps_per_block*threads_per_warp, 0, records[id_r]->cudaStream>>>(b->data_gpu, flags_gpu, records[id_r]->map_idxs_gpu, records[id_r]->dist_keys_gpu, records[id_r]->start_idxs_gpu, records[id_r]->state_ptrs_gpu, (b->kb).num_dist_keys, num_active_thread_per_warp);
#else
        Stateful_Processing_Kernel<<<num_blocks, warps_per_block*threads_per_warp, sizeof(Window_State) * num_active_thread_per_warp * warps_per_block*, records[id_r]->cudaStream>>>(b->data_gpu, flags_gpu, records[id_r]->map_idxs_gpu, records[id_r]->dist_keys_gpu, records[id_r]->start_idxs_gpu, records[id_r]->state_ptrs_gpu, (b->kb).num_dist_keys, num_active_thread_per_warp);
#endif
        batch_to_be_sent = b;
        id_r = (id_r + 1) % 2;
...
int main(int argc, char * argv[]) {
    ...
    if (argc != 9) {
        cout << argv[0] << " -s [num sources] -k [num keys] -b [batch length] -n [map degree]" << endl;
        exit(EXIT_SUCCESS);
    }
    while ((option = getopt(argc, argv, "s:k:b:n:")) != -1) {
        switch (option) {
            case 's': num_sources = atoi(optarg);
                     break;
	...
	bool *flags_gpu = nullptr;
	cudaMalloc(&flags_gpu, sizeof(bool) * batch_size);
	for (size_t i=0; i<map_degree; i++) {
		ff_comb *comb_node = new ff_comb(new dummy_mi(), new Filter(i, map_degree, app_start_time, batch_size, flags_gpu), true, true);
