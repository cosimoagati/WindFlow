...
/*  
 *  Version with { Sources } -> { Map } -> Sink. Stateless Map on GPU.
 */ 
...
public:
				...
				mask(_n_keys-1)
	{
    	...
    	std::uniform_int_distribution<std::mt19937::result_type> dist(0, 1221);
    	mt19937 rng;
    	rng.seed(0);
		// populate the table
...
public:
        ...
        mapF(_num_keys)
    {
        ...
        gpuErrChk(cudaMemcpyAsync(mapF_gpu, &mapF, sizeof(Map_Functor), cudaMemcpyHostToDevice, cudaStream));
    }
    ...
    batch_t<tuple_t, size_t> *svc(batch_t<tuple_t, size_t> *b)
    {
		...
		Stateless_Processing_Kernel<<<num_blocks, warps_per_block*threads_per_warp, 0, cudaStream>>>(b->data_gpu, b->size, mapF, num_active_thread_per_warp);
...
int main(int argc, char * argv[]) {
	...
	for (size_t i=0; i<map_degree; i++) {
		ff_comb *comb_node = new ff_comb(new dummy_mi(), new Map(i, map_degree, app_start_time, batch_size, num_keys), true, true);
