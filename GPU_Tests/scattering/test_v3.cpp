...
public:
    ...
    batch_t *svc(batch_t *b)
    {
        ...
        for (size_t w=0; w<n_dest; w++) {
            ...
            thrust::copy_if(thrust::cuda::par.on(cudaStream), bin_keys_gpu, bin_keys_gpu + b->size, bout_keys_gpu, is_routed(w, n_dest));
            // copy the right data directed to w
            auto end = thrust::copy_if(thrust::cuda::par.on(cudaStream), bin_data_gpu, bin_data_gpu + b->size, bin_keys_gpu, bout_data_gpu, is_routed(w, n_dest));
            gpuErrChk(cudaStreamSynchronize(cudaStream));
