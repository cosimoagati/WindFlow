...
void mapped_scan(int *output, size_t *input, int size, int n_dest, int target_value, cudaStream_t *cudaStream) {
	...
	thrust::inclusive_scan(thrust::cuda::par.on(*cudaStream), t_output, t_output+size, t_output); // in-place scan
}
	...
	batch_t *svc(batch_t *b) {
		...
		for (size_t w=0; w<n_dest; w++) {
			...
			if (bout_size > 0) {
				...
				create_sub_batch<tuple_t><<<num_of_blocks, threads_per_block, 0, cudaStream>>>
					(b->data_gpu, b->keys_gpu, b->size, scan_gpu, b_tosend->data_gpu,
					 b_tosend->keys_gpu, n_dest, w);
				assert(cudaGetLastError() == cudaSuccess);
				gpuErrChk(cudaStreamSynchronize(cudaStream));
