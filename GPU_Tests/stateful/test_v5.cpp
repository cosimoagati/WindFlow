                                           ...
                                           int num_active_thread_per_warp)
{
    ...
    int id_worker = id / threads_per_worker; // id of the worker
...
public:
    ...
    batch_t *svc(batch_t *b)
    {
        ...
        gpuErrChk(cudaMemcpyAsync(state_ptrs_gpu, state_ptrs_cpu, b->size * sizeof(state_t *), cudaMemcpyHostToDevice, cudaStream));
        ...
        Stateful_Processing_Kernel<<<num_blocks, warps_per_block*threads_per_warp, 0, cudaStream>>>(b->data_gpu, b->keys_gpu, dist_keys_gpu, num_dist_keys, state_ptrs_gpu, b->size, num_active_thread_per_warp);
        free(state_ptrs_cpu);
        free(dist_keys_cpu);
        volatile unsigned long end_time_nsec = current_time_nsecs();
        unsigned long elapsed_time_nsec = end_time_nsec - start_time_nsec;
