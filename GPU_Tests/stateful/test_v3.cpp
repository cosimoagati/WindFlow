...
public:
	...
	batch_t *svc(batch_t *b)
	{
		...
		int warps_per_block = std::min((int) ceil(((double) num_dist_keys) / num_blocks), ((max_threads_per_sm / max_blocks_per_sm) / threads_per_warp));
		Stateful_Processing_Kernel<<<num_blocks, warps_per_block*threads_per_warp, 0, cudaStream>>>(b->data_gpu, b->keys_gpu, dist_keys_gpu, num_dist_keys, state_ptrs_gpu, b->size);
		free(state_ptrs_cpu);
		free(dist_keys_cpu);
