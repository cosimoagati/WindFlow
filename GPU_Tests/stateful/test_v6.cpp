...
/*  
 *  Test of the stateful batch computation (version 6). This implementation is
 *  similar to version 5 because it overlaps the preparation of the next batch
 *  with the computation of the previous one on the GPU. Furthermore, similar to
 *  version 4, the number of active threads per warp depends on the number of
 *  unique keys in the input batch.
 *  In this version, we use a different approach that avoids scanning the whole
 *  array to look for inputs whose key is assigned to the current worker. Each
 *  worker on the GPU jumps exactly to the right positions in the batch where are
...
__global__ void Stateful_Processing_Kernel(tuple_t *tuples,
                                           ...
                                           int *map_idxs,
                                           size_t len,
                                           size_t *dist_keys_gpu,
                                           size_t *start_idxs,
                                           state_t **states,
                                           size_t num_dist_keys,
                                           int num_active_thread_per_warp)
{
	...
	if (id % threads_per_worker == 0) {
		...
		for (size_t id_key=id_worker; id_key<num_dist_keys; id_key+=num_workers) {
			...
			while (idx != -1) {
				process(&(tuples[idx]), states[id_key]);
				idx = map_idxs[idx];
			}
...
public:
	...
	batch_t *create_batch()
	{
		...
		gpuErrChk(cudaMemcpyAsync(b->data_gpu, data_cpu, batch_len * sizeof(tuple_t), cudaMemcpyHostToDevice, cudaStream));
...
public:
	...
	batch_t *svc(batch_t *b)
	{
		...
		gpuErrChk(cudaMemcpyAsync(b->keys_cpu, b->keys_gpu, sizeof(size_t) * b->size, cudaMemcpyDeviceToHost, cudaStream));
		...
		for (size_t i=0; i<num_found_dests; i++) {
			...
			batch_t *bout = new batch_t(result, b->data_gpu, b->keys_gpu, b->keys_cpu, offset, counter);
...
public:
		...
		id(_id)
	{
		...
		if (id == 0) {
			cout << "NumSMs: " << numSMs << ", threads per SM: " << max_threads_per_sm << ", blocks per SM: " << max_blocks_per_sm << ", threads per warp " << threads_per_warp << endl;
		}
	}
	...
	batch_t *svc(batch_t *b)
	{
		...
		for (size_t i=0; i<b->size; i++) {
			...
			if (it2 == dist_map.end()) {
				...
				start_idxs_cpu[num_dist_keys] = i;
				state_ptrs_cpu[num_dist_keys] = (*it).second;
				num_dist_keys++;
			}
			else {
				map_idxs_cpu[(*it2).second] = i;
				(*it2).second = i;
			}
		}
		...
		gpuErrChk(cudaMemcpyAsync(state_ptrs_gpu, state_ptrs_cpu, num_dist_keys * sizeof(state_t *), cudaMemcpyHostToDevice, cudaStream));
		gpuErrChk(cudaMemcpyAsync(map_idxs_gpu, map_idxs_cpu, b->size * sizeof(int), cudaMemcpyHostToDevice, cudaStream));
		gpuErrChk(cudaMemcpyAsync(dist_keys_gpu, dist_keys_cpu, num_dist_keys * sizeof(size_t), cudaMemcpyHostToDevice, cudaStream));
		gpuErrChk(cudaMemcpyAsync(start_idxs_gpu, start_idxs_cpu, num_dist_keys * sizeof(size_t), cudaMemcpyHostToDevice, cudaStream));
		...
		int num_blocks = std::min((int) ceil(((double) num_dist_keys) / warps_per_block), numSMs * max_blocks_per_sm);
		cudaFreeHost(dist_keys_cpu);
		cudaFreeHost(state_ptrs_cpu);
		cudaFreeHost(map_idxs_cpu);
		cudaFreeHost(start_idxs_cpu);
		Stateful_Processing_Kernel<<<num_blocks, warps_per_block*threads_per_warp, /*sizeof(state_t) * num_active_thread_per_warp * warps_per_block*/ 0, cudaStream>>>(b->data_gpu, b->keys_gpu, map_idxs_gpu, b->size, dist_keys_gpu, start_idxs_gpu, state_ptrs_gpu, num_dist_keys, num_active_thread_per_warp);
		volatile unsigned long end_time_nsec = current_time_nsecs();
		unsigned long elapsed_time_nsec = end_time_nsec - start_time_nsec;
		tot_elapsed_nsec += elapsed_time_nsec;
		batch_to_be_sent = b;
	...
	void svc_end()
	{
		...
		//std::cout << "[Worker] average service time: " << (((double) tot_elapsed_nsec)/received_batch) / 1000 << " usec" << std::endl;
	}
};
...
public:
	...
	// constructor
	Sink(): start_time_us(current_time_usecs()),
