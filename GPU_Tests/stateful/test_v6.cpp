...
__global__ void Stateful_Processing_Kernel(tuple_t *tuples, size_t *keys, int *map_idxs, size_t len,
...
public:
	...
	batch_t *svc(batch_t *b) {
		...
		Stateful_Processing_Kernel<<<
		        ...
		        b->data_gpu, b->keys_gpu, map_idxs_gpu, b->size, dist_keys_gpu, start_idxs_gpu,
		        state_ptrs_gpu, num_dist_keys, num_active_thread_per_warp);
		volatile unsigned long end_time_nsec     = current_time_nsecs();
		unsigned long          elapsed_time_nsec = end_time_nsec - start_time_nsec;
		tot_elapsed_nsec += elapsed_time_nsec;
		batch_to_be_sent = b;
