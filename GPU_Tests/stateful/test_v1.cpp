...
public:
	...
	batch_t *svc(batch_t *b) {
		...
		gpuErrChk(cudaMemcpyAsync(state_ptrs_gpu, state_ptrs_cpu,
					  b->size * sizeof(state_t *), cudaMemcpyHostToDevice, cudaStream));
		// proces the batch in a stateful manner
		size_t num_blocks = (size_t) ceil(((double) b->size) / threads_per_block); // it can be too large, no check here...
