...
struct batch_t {
	...
	// constructor II
	batch_t(size_t _size, tuple_t *_data_gpu, size_t *_keys_gpu, size_t _offset, atomic<size_t> *_counter)
	{
		size = _size;
...
public:
	...
	Source(long _stream_len, long _num_keys, long _batch_len)
		: stream_len(_stream_len), num_keys(_num_keys), batch_len(_batch_len),
		  dist(0, _num_keys-1), dist2(0, 2000)
	{
		// set random seed
...
public:
	...
	batch_t *svc(batch_t *b) {
		...
		gpuErrChk(cudaMemcpyAsync(unique_dests_cpu,
					  thrust::raw_pointer_cast(unique_dests_gpu.data()),
					  sizeof(size_t) * num_found_dests,
					  cudaMemcpyDeviceToHost, cudaStream));
		gpuErrChk(cudaMemcpyAsync(freqs_dests_cpu,
					  thrust::raw_pointer_cast(freqs_dests_gpu.data()),
...
public:
	...
	Worker(size_t _par_deg, size_t _id) : par_deg(_par_deg), id(_id) {
		...
		if (id == 0)
			cout << "NumSMs: " << numSMs << ", threads per SM: " << max_threads_per_sm << ", blocks per SM: " << max_blocks_per_sm << endl;
	}
	...
	batch_t *svc(batch_t *b) {
		...
		gpuErrChk(cudaMemcpyAsync(keys_cpu, b->keys_gpu,
					  b->size * sizeof(size_t), cudaMemcpyDeviceToHost, cudaStream));
		gpuErrChk(cudaStreamSynchronize(cudaStream));
#if 1
		...
		size_t tot_num_threads = std::min((int) num_dist_keys, max_threads_per_sm * numSMs);
		size_t threads_per_block = max_threads_per_sm / max_blocks_per_sm; // 128
		size_t num_blocks = ceil(((double) tot_num_threads) / threads_per_block);
