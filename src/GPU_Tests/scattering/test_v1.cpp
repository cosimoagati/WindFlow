...
/*
 *  Test of the scatter functionality (version 1). For each input batch, the
 ...
 *  destination connected to the emitter. Both the scan and the sub-batch creation
 *  are done on the GPU in parallel.
 */
...
struct batch_t {
	...
	void print(cudaStream_t &cudaStream) {
		...
		gpuErrChk(cudaMemcpyAsync(keys_cpu, keys_gpu, size * sizeof(size_t), cudaMemcpyDeviceToHost,
		                          cudaStream));
...
__global__ void map_to_target(int *output, size_t *input, int size, int n_dest, int target_value) {
...
void mapped_scan(int *output, size_t *input, int size, int n_dest, int target_value,
                 cudaStream_t *cudaStream) {
	auto num_of_blocks = ceil(((double)size) / threads_per_block);
	map_to_target<<<num_of_blocks, threads_per_block, 0, *cudaStream>>>(output, input, size, n_dest,
	                                                                    target_value);
	assert(cudaGetLastError() == cudaSuccess);
	thrust::device_ptr<int> t_output = thrust::device_pointer_cast(output);
	thrust::inclusive_scan(thrust::cuda::par.on(*cudaStream), t_output, t_output + size,
	                       t_output); // in-place scan
}

// create_sub_batch kernel
template<typename input_t>
__global__ void create_sub_batch(input_t *bin, size_t *keys, size_t batch_size, int *scan, input_t *bout,
                                 size_t *keys_out, int n_dest, int target_node) {
...
public:
	...
	Source(long _stream_len, long _num_keys, long _batch_len)
	        : stream_len(_stream_len), num_keys(_num_keys), batch_len(_batch_len),
	          dist(0, _num_keys - 1) {
		// set random seed
...
public:
	...
	batch_t *svc(batch_t *b) {
		...
		for (size_t w = 0; w < n_dest; w++) {
			mapped_scan(scan_gpu, b->keys_gpu, b->size, n_dest, w, &cudaStream);
			gpuErrChk(cudaMemcpyAsync(bout_size, &(scan_gpu[b->size - 1]), sizeof(int),
			                          cudaMemcpyDeviceToHost, cudaStream));
			gpuErrChk(cudaStreamSynchronize(cudaStream));
			if (bout_size > 0) {
				batch_t *b_tosend      = new batch_t(*bout_size);
				auto     num_of_blocks = ceil(((double)b->size) / threads_per_block);
				create_sub_batch<tuple_t>
				        <<<num_of_blocks, threads_per_block, 0, cudaStream>>>(
				                b->data_gpu, b->keys_gpu, b->size, scan_gpu,
				                b_tosend->data_gpu, b_tosend->keys_gpu, n_dest, w);
				assert(cudaGetLastError() == cudaSuccess);
				gpuErrChk(cudaStreamSynchronize(cudaStream));
...
public:
	...
	// constructor
	Sink()
	        : received(0), received_sample(0), start_time_us(current_time_usecs()),
