...
__global__ void Initialize_States_Kernel(Window_State **new_states, size_t num_states) {
	const int thread_id   = threadIdx.x + blockIdx.x * blockDim.x;
	const int num_threads = gridDim.x * blockDim.x;
	for (size_t i = thread_id; i < num_states; i += num_threads)
...
public:
	Filter(size_t _id_map, size_t _map_degree, const unsigned long _app_start_time, size_t _max_batch_len,
	       bool *_flags_gpu)
	        : id_map(_id_map), map_degree(_map_degree), processed(0), app_start_time(_app_start_time),
	          current_time(_app_start_time), max_batch_len(_max_batch_len), flags_gpu(_flags_gpu) {
	...
	batch_t<tuple_t, size_t> *svc(batch_t<tuple_t, size_t> *b) {
		...
		const int num_blocks =
		        std::min((int) ceil(((double) (b->kb).num_dist_keys) / threads_per_block),
		                 numSMs * max_blocks_per_sm);
		if (batch_to_be_sent != nullptr) {
			...
			if (batch_to_be_sent->isKBDone()) {
				...
				thrust::device_ptr<bool> th_flags_gpu =
				        thrust::device_pointer_cast(flags_gpu);
				thrust::device_ptr<tuple_t> th_data_gpu =
				        thrust::device_pointer_cast(batch_to_be_sent->data_gpu);
				thrust::device_ptr<tuple_t> th_new_data_gpu =
				        thrust::device_pointer_cast(new_data_gpu);
				auto pred = [] __device__(bool x) { return x; };
				auto end  = thrust::copy_if(
                                        thrust::cuda::par(alloc).on(records[(id_r + 1) % 2]->cudaStream),
                                        th_data_gpu, th_data_gpu + batch_to_be_sent->size, flags_gpu,
                                        th_new_data_gpu, pred);
				// change the logical size of the batch
				batch_to_be_sent->size = end - th_new_data_gpu;
				gpuErrChk(cudaMemcpyAsync(batch_to_be_sent->data_gpu, new_data_gpu,
				                          batch_to_be_sent->size * sizeof(tuple_t),
				                          cudaMemcpyDeviceToDevice,
				                          records[(id_r + 1) % 2]->cudaStream));
				gpuErrChk(cudaStreamSynchronize(records[(id_r + 1) % 2]->cudaStream));
...
#if !defined(__SHARED__)
		Stateful_Processing_Kernel<<<num_blocks, threads_per_block, 0, records[id_r]->cudaStream>>>(
		        b->data_gpu, flags_gpu, records[id_r]->map_idxs_gpu, records[id_r]->start_idxs_gpu,
		        records[id_r]->state_ptrs_gpu, (b->kb).num_dist_keys);
#else
		Stateful_Processing_Kernel<<<num_blocks, threads_per_block,
		                             sizeof(Window_State) * threads_per_block,
		                             records[id_r]->cudaStream>>>(
		        b->data_gpu, flags_gpu, records[id_r]->map_idxs_gpu, records[id_r]->start_idxs_gpu,
		        records[id_r]->state_ptrs_gpu, (b->kb).num_dist_keys);
#endif
...
int main(int argc, char *argv[]) {
	...
	bool *            flags_gpu = nullptr;
	gpuErrChk(cudaMalloc(&flags_gpu, sizeof(bool) * batch_size));
	for (size_t i = 0; i < map_degree; i++) {
		ff_comb *comb_node = new ff_comb(
		        new dummy_mi(), new Filter(i, map_degree, app_start_time, batch_size, flags_gpu),
		        true, true);
