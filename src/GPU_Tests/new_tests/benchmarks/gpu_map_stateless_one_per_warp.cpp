...
public:
	Map_Functor(size_t _n_keys) : size(_n_keys), mask(_n_keys - 1) {
	...
	Map(size_t _id_map, size_t _map_degree, const unsigned long _app_start_time, size_t _max_batch_len,
	    size_t _num_keys)
	        : id_map(_id_map), map_degree(_map_degree), processed(0), app_start_time(_app_start_time),
	          current_time(_app_start_time), max_batch_len(_max_batch_len), mapF(_num_keys) {
	...
	batch_t<tuple_t, size_t> *svc(batch_t<tuple_t, size_t> *b) {
		...
		if (received < 100 && b->size > 0) {
			...
			for (size_t i = 0; i < b->size; i++) {
				...
				cout << "Tuple: " << t->key << " " << t->property_value << " " << t->id
				     << endl;
				if (received + i >= 100)
					break;
			}
...
int main(int argc, char *argv[]) {
	...
	for (size_t i = 0; i < map_degree; i++) {
		ff_comb *comb_node =
		        new ff_comb(new dummy_mi(),
		                    new Map(i, map_degree, app_start_time, batch_size, num_keys), true, true);
