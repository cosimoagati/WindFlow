...
__global__ void Stateful_Processing_Kernel(tuple_t *tuples, bool *flags, int *map_idxs, int *start_idxs,
	...
	const int num_workers = num_threads / warpSize;
	const int worker_id   = thread_id / warpSize;
	// only one thread each warpSize threads works, the others are idle
