...
public:
	...
	{
		return std::make_unique<source_t>(func, pardegree, name,
						  closing_func);
	}
};
...
public:
	...
	filter_t build()
	{
		if (!isKeyed)
			...
			return filter_t(func, pardegree, name, closing_func,
					routing_func); // copy elision in C++17
	}
#endif
	...
	{
		if (!isKeyed)
			return std::make_unique<filter_t>(func, pardegree,
							  name, closing_func);
		else
			return std::make_unique<filter_t>(func, pardegree,
							  name,
...
public:
	...
	map_t build()
	{
		if (!isKeyed)
			...
			return map_t(func, pardegree, name, closing_func,
				     routing_func); // copy elision in C++17
	}
#endif
	...
	{
		if (!isKeyed)
			return std::make_unique<map_t>(func, pardegree,
						       name,
						       closing_func);
		else
			return std::make_unique<map_t>(func, pardegree, name,
						       closing_func,
						       routing_func);
	}
};
...
public:
	...
	flatmap_t build()
	{
		if (!isKeyed)
			...
			return flatmap_t(func, pardegree, name, closing_func,
					 routing_func); // copy elision in C++17
	}
#endif
	...
	{
		if (!isKeyed)
			return std::make_unique<flatmap_t>(func, pardegree,
							   name,
							   closing_func);
		else
			return std::make_unique<flatmap_t>(func, pardegree,
...
private:
	F_t func;
	// type of the operator to be created by this builder
	using accumulator_t = Accumulator<decltype(get_tuple_t(func)),
					  decltype(get_result_t(func))>;
	// type of the closing function
	using closing_func_t = std::function<void(RuntimeContext&)>;
	// type of the function to map the key hashcode onto an identifier starting from zero to pardegree-1
...
public:
	...
	accumulator_t build()
	{
		return accumulator_t(func, init_value, pardegree, name,
				     closing_func,
				     routing_func); // copy elision in C++17
	}
#endif
	...
	{
		return new accumulator_t(func, init_value, pardegree, name,
					 closing_func,
					 routing_func);
	}
...
public:
	 ...
	 */
	PaneFarmGPU_Builder(F_t _func_F, G_t _func_G): func_F(_func_F),
						       func_G(_func_G) {}
...
public:
	 ...
	 */
	WinMapReduce_Builder(F_t _func_F, G_t _func_G): func_F(_func_F),
							func_G(_func_G) {}
...
public:
	 ...
	 *  The GPU function must be passed through a callable object (e.g., lambda, functor)
	 */
	WinMapReduceGPU_Builder(F_t _func_F, G_t _func_G): func_F(_func_F),
							   func_G(_func_G) {}
...
public:
	...
	sink_t build()
	{
		if (!isKeyed)
			...
			return sink_t(func, pardegree, name, closing_func,
				      routing_func); // copy elision in C++17
	}
#endif
	...
	{
		if (!isKeyed)
			return std::make_unique<sink_t>(func, pardegree, name,
							closing_func);
		else
			return std::make_unique<sink_t>(func, pardegree, name,
							closing_func,
