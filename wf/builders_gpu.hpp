 ...
 *  @author  Gabriele Mencagli, Cosimo Agati
 *  @date    02/04/2020
 *  
...
private:
	...
	static_assert(!(std::is_same<tuple_t, std::false_type>::value
			|| std::is_same<result_t, std::false_type>::value),
		      ...
		      "  Candidate : __host__ __device__ void(std::uint64_t, const tuple_t *, std::size_t, result_t *, char *, std::size_t)\n");
...
public:
	 ...
	 *  \return the object itself
	 */
	WinSeqGPU_Builder<F_t> &withGPUConfiguration(int _gpu_id=0,
...
public:
	 ...
	 */
	WinSeqFFATGPU_Builder(F_t _lift_func, G_t _comb_func)
	 ...
	 *  \return the object itself
	 */
	WinSeqFFATGPU_Builder<F_t, G_t> &withCBWindows(std::uint64_t _win_len, std::uint64_t _slide_len) {
		win_len = _win_len;
		slide_len = _slide_len;
		winType = CB;
		return *this;
	 ...
	 *  \return the object itself
	 */
	WinSeqFFATGPU_Builder<F_t, G_t> &withBatch(std::size_t _batch_len) {
		batch_len = _batch_len;
		return *this;
	}
	 ...
	 *  \return the object itself
	 */
	WinSeqFFATGPU_Builder<F_t, G_t> &withGPUConfiguration(int _gpu_id=0,
							      std::size_t _n_thread_block=DEFAULT_CUDA_NUM_THREAD_BLOCK) {
		gpu_id = _gpu_id;
		n_thread_block = _n_thread_block;
		return *this;
	...
	winffat_gpu_t *build_ptr() {
		return new winffat_gpu_t(lift_func, comb_func, win_len,
					 slide_len, triggering_delay, winType,
					 batch_len, gpu_id, n_thread_block,
					 rebuild, name);
	}
	...
	std::unique_ptr<winffat_gpu_t> build_unique() {
		return std::make_unique<winffat_gpu_t>(lift_func, comb_func,
						       win_len, slide_len,
						       triggering_delay,
						       winType, batch_len,
						       gpu_id, n_thread_block,
						       rebuild, name);
	}
};
...
private:
	...
	static_assert(!std::is_same<winfarm_gpu_t, std::false_type>::value,
		      ...
		      "  Candidate 1 : __host__ __device__ void(std::uint64_t, const tuple_t *, std::size_t, result_t *, char *, std::size_t)\n"
...
public:
	 ...
	 *  \return the object itself
	 */
	WinFarmGPU_Builder<T> &withCBWindows(std::uint64_t _win_len, std::uint64_t _slide_len) {
	 ...
	 *  \return the object itself
	 */
	WinFarmGPU_Builder<T> &withGPUConfiguration(int _gpu_id=0,
						    std::size_t _n_thread_block=DEFAULT_CUDA_NUM_THREAD_BLOCK) {
		// check if it is a complex nesting
		if (isComplex) {
			std::cerr << RED << "WindFlow Error: nesting does not allow configuring the Win_Farm_GPU parameters" << DEFAULT_COLOR << std::endl;
...
public:
	 ...
	 *  \return the object itself
	 */
	KeyFarmGPU_Builder<T> &withGPUConfiguration(int _gpu_id=0,
						    std::size_t _n_thread_block=DEFAULT_CUDA_NUM_THREAD_BLOCK) {
		// check if it is a complex nesting
		if (isComplex) {
			std::cerr << RED << "WindFlow Error: nesting does not allow configuring the Key_Farm_GPU parameters" << DEFAULT_COLOR << std::endl;
	...
	keyfarm_gpu_t *build_ptr() {
		return new keyfarm_gpu_t(input, win_len, slide_len,
					 triggering_delay, winType, pardegree,
					 batch_len, gpu_id, n_thread_block,
					 name, scratchpad_size, routing_func,
					 opt_level);
	}
...
public:
	 ...
	 *  \return the object itself
	 */
	KeyFFATGPU_Builder<F_t, G_t> &withCBWindows(std::uint64_t _win_len, std::uint64_t _slide_len) {
		win_len = _win_len;
		slide_len = _slide_len;
		winType = CB;
		return *this;
	 ...
	 *  \return the object itself
	 */
	KeyFFATGPU_Builder<F_t, G_t> &withParallelism(std::size_t _pardegree) {
		pardegree = _pardegree;
		return *this;
	}
	 ...
	 *  \return the object itself
	 */
	KeyFFATGPU_Builder<F_t, G_t> &withBatch(std::size_t _batch_len) {
		batch_len = _batch_len;
		return *this;
	}
	 ...
	 *  \return the object itself
	 */
	KeyFFATGPU_Builder<F_t, G_t> &withGPUConfiguration(int _gpu_id=0,
							   std::size_t _n_thread_block=DEFAULT_CUDA_NUM_THREAD_BLOCK) {
		gpu_id = _gpu_id;
		n_thread_block = _n_thread_block;
		return *this;
	...
	keyffat_gpu_t *build_ptr() {
		return new keyffat_gpu_t(lift_func, comb_func, win_len,
					 slide_len, triggering_delay, winType,
					 pardegree, batch_len, gpu_id,
					 n_thread_block, rebuild, name,
					 routing_func);
	}
	...
	std::unique_ptr<keyffat_gpu_t> build_unique() {
		return std::make_unique<keyffat_gpu_t>(lift_func, comb_func,
						       win_len, slide_len,
						       triggering_delay,
						       winType, pardegree,
						       batch_len, gpu_id,
						       n_thread_block, rebuild,
						       name, routing_func);
	}
};
...
private:
	...
	// static assert to check the signature
	static_assert(!(std::is_same<tuple_t1, std::false_type>::value
			&& std::is_same<result_t1, std::false_type>::value
			&& std::is_same<tuple_t2, std::false_type>::value
			&& std::is_same<result_t2, std::false_type>::value),
		      "WindFlow Compilation Error - unknown signature passed to the PaneFarmGPU_Builder (first argument, PLQ logic):\n"
		      "  Candidate 1 : void(std::uint64_t, const Iterable<tuple_t> &, result_t &)\n"
		      "  Candidate 2 : void(std::uint64_t, const tuple_t &, result_t &)\n"
	...
	static_assert(!(std::is_same<std::false_type, result_t3>::value
			&& std::is_same<std::false_type, result_t4>::value),
		      "WindFlow Compilation Error - unknown signature passed to the PaneFarmGPU_Builder (second argument, WLQ logic):\n"
		      "  Candidate 1 : void(std::uint64_t, const Iterable<result_t> &, result_t &)\n"
		      "  Candidate 2 : void(std::uint64_t, const result_t &, result_t &)\n"
		      "  , or if the WLQ stage is on GPU\n"
		      "  Candidate : __host__ __device__ void(std::uint64_t, const result_t *, std::size_t, result_t *, char *, std::size_t)\n");
	static_assert(!((!std::is_same<result_t2, std::false_type>::value
			 && !std::is_same<result_t4, std::false_type>::value)
			|| (!std::is_same<result_t1, std::false_type>::value
			    && !std::is_same<result_t3, std::false_type>::value)),
		      "WindFlow Compilation Error - exactly one of the two stages (either PLQ or WLQ) must be on GPU!\n");
	static_assert(std::is_same<result_t1, result_t4>::value
		      && std::is_same<result_t2, result_t3>::value,
	...
	using tuple_t_plq = typename std::conditional<!std::is_same<tuple_t1, std::false_type>::value,
						      tuple_t1, tuple_t2>::type;
	using result_t_plq = typename std::conditional<!std::is_same<result_t1, std::false_type>::value,
						       result_t1, result_t2>::type;
...
public:
	 ...
	 *  \return the object itself
	 */
	PaneFarmGPU_Builder<F_t, G_t> &withCBWindows(std::uint64_t _win_len, std::uint64_t _slide_len) {
		win_len = _win_len;
		slide_len = _slide_len;
		winType = CB;
		return *this;
	 ...
	 *  \return the object itself
	 */
	PaneFarmGPU_Builder<F_t, G_t> &withParallelism(std::size_t _plq_degree, std::size_t _wlq_degree) {
	 ...
	 *  \return the object itself
	 */
	PaneFarmGPU_Builder<F_t, G_t> &withGPUConfiguration(int _gpu_id=0,
							    std::size_t _n_thread_block=DEFAULT_CUDA_NUM_THREAD_BLOCK) {
		gpu_id = _gpu_id;
		n_thread_block = _n_thread_block;
		return *this;
...
private:
	...
	using tuple_t_map = typename std::conditional<!std::is_same<tuple_t1, std::false_type>::value,
						      tuple_t1, tuple_t2>::type;
	using result_t_map = typename std::conditional<!std::is_same<result_t1, std::false_type>::value,
						       result_t1, result_t2>::type;
	using gpu_func_t = decltype(get_GPU_F(func_F, func_G));
	static_assert(!std::is_same<gpu_func_t, std::false_type>::value,
...
public:
	 ...
	 *  \return the object itself
	 */
	WinMapReduceGPU_Builder<F_t, G_t> &withCBWindows(std::uint64_t _win_len, std::uint64_t _slide_len) {
		win_len = _win_len;
		slide_len = _slide_len;
		winType = CB;
		return *this;
	 ...
	 *  \return the object itself
	 */
	WinMapReduceGPU_Builder<F_t, G_t> &withParallelism(std::size_t _map_degree, std::size_t _reduce_degree) {
	 ...
	 *  \return the object itself
	 */
	WinMapReduceGPU_Builder<F_t, G_t> &withGPUConfiguration(int _gpu_id=0,
								std::size_t _n_thread_block=DEFAULT_CUDA_NUM_THREAD_BLOCK) {
		gpu_id = _gpu_id;
		n_thread_block = _n_thread_block;
		return *this;
	...
	winmapreduce_gpu_t *build_ptr() {
		return new winmapreduce_gpu_t(func_F, func_G, win_len,
					      ...
					      winType, map_degree,
					      reduce_degree, batch_len, gpu_id,
					      n_thread_block, name,
...
public:
	...
	MapGPU_Builder<F_t> &withKeyBy(const bool is_keyed) {
		routing_mode = is_keyed ? KEYBY : FORWARD;
		return *this;
	}
	 ...
	 *  \return the object itself.
	 */
	MapGPU_Builder<F_t> &withTupleBufferCapacity(const std::size_t capacity) {
		this->tuple_buffer_capacity = capacity;
		return *this;
	}
	...
	mapgpu_t build() {
		...
		if (!is_keyed) {
			return mapgpu_t {func, pardegree, name,
					 tuple_buffer_capacity,
					 gpu_threads_per_block}
		}
		return mapgpu_t {func, pardegree, name, routing_func,
				 tuple_buffer_capacity,
				 gpu_threads_per_block,
				 scratchpad_size};
	}
