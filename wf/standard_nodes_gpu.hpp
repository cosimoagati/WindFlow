...
__global__ void create_sub_batch(tuple_t *const bin, const std::size_t batch_size, std::size_t *const index,
                                 std::size_t *const scan, tuple_t *const bout, const int target_node) {
	const auto id = blockIdx.x * blockDim.x + threadIdx.x;
	if (id < batch_size && index[id] == target_node)
		bout[scan[index[id]] - 1] = bin[id];
}

template<typename tuple_t> class Standard_EmitterGPU : public Basic_Emitter {
private:
...
public:
	Standard_EmitterGPU(const std::size_t num_of_destinations, const bool have_gpu_input = false,
	                    const int gpu_threads_per_block = 256)
	        : routing_mode {FORWARD}, num_of_destinations {num_of_destinations},
	          have_gpu_input {have_gpu_input}, gpu_threads_per_block {gpu_threads_per_block},
	          gpu_blocks {std::ceil(num_of_destinations / static_cast<float>(gpu_threads_per_block))} {
		if (num_of_destinations <= 0)
			failwith("Standard_EmitterGPU initialized with non-positive number of destinations.");
	}

	Standard_EmitterGPU(const routing_func_t routing_func, const std::size_t num_of_destinations,
	                    const bool have_gpu_input = false, const int gpu_threads_per_block = 256)
	        : routing_mode {KEYBY}, routing_func {routing_func},
	          num_of_destinations {num_of_destinations}, cpu_tuple_buffer(num_of_destinations),
	          cpu_hash_index(num_of_destinations),
	          gpu_hash_index {num_of_destinations}, scan {num_of_destinations},
	          have_gpu_input {have_gpu_input}, gpu_threads_per_block {gpu_threads_per_block},
	          gpu_blocks {std::ceil(num_of_destinations / static_cast<float>(gpu_threads_per_block))} {
		if (num_of_destinations <= 0)
			failwith("Standard_EmitterGPU initialized with non-positive number of destinations.");
	...
	Basic_Emitter *clone() const { return new Standard_EmitterGPU<tuple_t> {*this}; }
	...
	void linear_keyed_gpu_partition(const GPUBuffer<tuple_t> &handle) {
		...
		for (auto i = 0; i < num_of_destinations; ++i) {
			...
			cuda_error = cudaMemcpyAsync(sub_buffer.data(), cpu_sub_buffer.data(), raw_size,
	...
	void parallel_keyed_gpu_partition(const GPUBuffer<tuple_t> &handle) {
		...
		cuda_error = cudaMemcpyAsync(gpu_hash_index.data(), cpu_hash_index.data(), raw_index_size,
		                             cudaMemcpyHostToDevice, stream.raw());
		assert(cuda_error == cudaSuccess);
		...
		for (auto i = 0; i < num_of_destinations; ++i) {
			...
			create_sub_batch<<<gpu_blocks, gpu_threads_per_block, 0, stream.raw()>>>(
			        handle.data(), handle.size(), gpu_hash_index.data(), scan.data(), bout.data(),
			        i);
...
public:
	Standard_CollectorGPU(const ordering_mode_t mode = TS) { assert(mode == TS); }
