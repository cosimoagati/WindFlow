...
public:
	...
	using rich_acc_func_t = std::function<void(const tuple_t &, result_t &,
						   RuntimeContext &)>;
	...
	public:
		...
		result_t *svc(tuple_t *t)
		{
			...
			if (it == keyMap.end()) {
				...
				keyMap.insert(std::make_pair(key,
							     Key_Descriptor(init_value)));
				it = keyMap.find(key);
			}
...
public:
		    ...
		    routing_func_t _routing_func): used(false)
	{
		...
		for (size_t i=0; i<_pardegree; i++) {
			auto *seq =
				new Accumulator_Node(_func, _init_value, _name,
		    ...
		    routing_func_t _routing_func): used(false)
	{
		...
		// add default collector
		ff::ff_farm::add_collector(nullptr);
		// when the Accumulator will be destroyed we need aslo to
		// destroy the emitter, workers and collector
	 ff::ff_farm::cleanup_all();
	}
