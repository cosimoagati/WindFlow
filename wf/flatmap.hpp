...
public:
	...
	using flatmap_func_t = std::function<void(const tuple_t &,
						  Shipper<result_t> &)>;
	/// type of the rich flatmap function
	using rich_flatmap_func_t = std::function<void(const tuple_t &,
						       Shipper<result_t> &,
						       RuntimeContext &)>;
...
public:
		...
		keyed(false), used(false)
	{
		...
		if (_pardegree == 0) {
			std::cerr << RED
				  << "WindFlow Error: FlatMap has parallelism zero"
				  << DEFAULT_COLOR << std::endl;
			exit(EXIT_FAILURE);
		}
		...
		// when the FlatMap will be destroyed we need aslo to destroy
		// the emitter, workers and collector
