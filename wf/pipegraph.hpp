...
class selfkiller_node: public ff::ff_minode
{
	...
	void *svc(void *) { return this->EOS; /* terminates immediately! */ }
};
...
private:
	...
	MultiPipe(PipeGraph *_graph, std::vector<ff::ff_node *> _normalization)
		: graph(_graph), has_source(true), has_sink(false),
		  isMerged(false), isSplit(false), fromSplitting(false),
		  splittingBranches(0),
		  forceShuffling(true), // <-- we force a shuffling for the next operator
		  lastParallelism(0), outputType("")
	{
	...
	void add_operator(ff::ff_farm *_op, routing_types_t _type,
			  ordering_mode_t _ordering=TS);
	...
	if (parent_node != root) {
		for (auto *child: parent_node->children) {
			...
			for (auto *leaf: child_leaves) {
				if (std::find(inputNodes.begin(), inputNodes.end(),
					      leaf)
				    == inputNodes.end()) {
					foundAll = false;
				} else {
	...
	if (get_MergedNodes1(_toBeMerged, rightList)) {
		if (rightList.size() == 1) { // Case 2.1: merge-full -> we merge a whole sub-tree
			...
			for (auto *an: rightList) { // redundant check
				if (std::find((root->children).begin(),
					      (root->children).end(), an)
				    == (root->children).end()) {
			...
			for (auto *brother: root->children) {
				if (std::find(rightList.begin(),
					      rightList.end(), brother)
				    == rightList.end()) {
					children_new.push_back(brother);
				}
			}
...
MultiPipe &MultiPipe::add(Win_Farm<tuple_t, result_t> &_wf)
{
	...
	} else { 	// case with Win_Seq replicas inside
		if (_wf.getWinType() == TB) {
			// call the generic method to add the operator to the MultiPipe
			if (graph->mode == Mode::DETERMINISTIC) {
				add_operator<WF_Emitter<tuple_t>, Ordering_Node<tuple_t, wrapper_tuple_t<tuple_t>>>(&_wf, COMPLEX, TS);
...
MultiPipe &MultiPipe::add(Win_Farm_GPU<tuple_t, result_t, F_t> &_wf)
{
	...
	} else { 	// case with Win_Seq_GPU replicas inside
		if (_wf.getWinType() == TB) {
			// call the generic method to add the operator to the MultiPipe
			if (graph->mode == Mode::DETERMINISTIC) {
				add_operator<WF_Emitter<tuple_t>, Ordering_Node<tuple_t, wrapper_tuple_t<tuple_t>>>(&_wf, COMPLEX, TS);
