...
{
public:
	/// function type to map the key hashcode onto an identifier starting
	/// from zero to pardegree-1
	...
	using win_mapreduce_gpu_t = Win_MapReduce_GPU<tuple_t, result_t,
						      win_F_t>;
...
private:
	...
	// flag stating whether the Key_Farm_GPU has been instantiated with
	// complex workers (Pane_Farm_GPU or Win_MapReduce_GPU)
	...
	void optimize_KeyFarmGPU(opt_level_t opt)
	{
		if (opt == LEVEL0) // no optimization
			...
			kf_emitter_t *kf_e =
				static_cast<kf_emitter_t *>(this->getEmitter());
			...
			if (tobeTransformmed) {
				...
				auto *treeEmitter =
					new Tree_Emitter(kf_e, Es, true, true);
...
public:
		...
		used(false)
	{
		...
		if (_win_len == 0 || _slide_len == 0) {
			std::cerr << RED
				  << "WindFlow Error: window length or slide in Key_Farm_GPU cannot be zero"
		...
		if (_pardegree == 0) {
			std::cerr << RED
				  << "WindFlow Error: Key_Farm_GPU has parallelism zero"
				  << DEFAULT_COLOR << std::endl;
			exit(EXIT_FAILURE);
		}
		...
		if (_batch_len == 0) {
			std::cerr << RED
				  << "WindFlow Error: batch length in Key_Farm_GPU cannot be zero"
				  << DEFAULT_COLOR << std::endl;
			exit(EXIT_FAILURE);
		}
		...
		for (size_t i = 0; i < _pardegree; i++) {
			auto *seq = new win_seq_gpu_t(_win_func, _win_len,
						      _slide_len,
						      _triggering_delay,
						      _winType, _batch_len,
						      _n_thread_block,
		...
		ff::ff_farm::add_emitter(new kf_emitter_t(_routing_func,
							  _pardegree));
		...
		used(false)
	{
		...
		if (_pf.isUsed4Nesting()) {
			std::cerr << RED
				  << "WindFlow Error: Pane_Farm_GPU has already been used in a nested structure"
				  << DEFAULT_COLOR << std::endl;
			exit(EXIT_FAILURE);
		}
		else {
		...
		if (_pf.win_len != _win_len || _pf.slide_len != _slide_len
		    || _pf.triggering_delay != _triggering_delay
		    || _pf.winType != _winType || _pf.batch_len != _batch_len
		    || _pf.n_thread_block != _n_thread_block) {
			std::cerr << RED
				  << "WindFlow Error: incompatible windowing and batching parameters between Key_Farm_GPU and Pane_Farm_GPU"
				  << DEFAULT_COLOR << std::endl;
			exit(EXIT_FAILURE);
		}
		...
		for (size_t i = 0; i < _pardegree; i++) {
			...
			if (_pf.isGPUPLQ) {
				if (_pf.isNICWLQ) 
					pf_W = new pane_farm_gpu_t(_pf.gpuFunction, _pf.wlq_func,
								   _pf.win_len, _pf.slide_len,
								   _pf.triggering_delay, _pf.winType,
								   _pf.plq_degree, _pf.wlq_degree,
								   _pf.batch_len, _pf.n_thread_block,
								   _name + "_kf_" + std::to_string(i),
								   _pf.scratchpad_size, false,
								   _pf.opt_level, configPF);
				else
					pf_W = new pane_farm_gpu_t(_pf.gpuFunction,
								   _pf.wlqupdate_func,
								   _pf.win_len,
								   _pf.slide_len,
								   _pf.triggering_delay,
								   _pf.winType, _pf.plq_degree,
								   _pf.wlq_degree, _pf.batch_len,
								   _pf.n_thread_block,
								   _name + "_kf_" +
								   std::to_string(i),
								   _pf.scratchpad_size, false,
				...
				if (_pf.isNICPLQ)
					pf_W = new pane_farm_gpu_t(_pf.plq_func, _pf.gpuFunction,
								   _pf.win_len, _pf.slide_len,
								   _pf.triggering_delay, _pf.winType,
								   _pf.plq_degree, _pf.wlq_degree,
								   _pf.batch_len, _pf.n_thread_block,
					...
					pf_W = new pane_farm_gpu_t(_pf.plqupdate_func,
								   _pf.gpuFunction, _pf.win_len,
								   _pf.slide_len,
								   _pf.triggering_delay,
								   _pf.winType, _pf.plq_degree,
								   _pf.wlq_degree, _pf.batch_len,
		...
		used(false)
	{
		...
		if (_wm.isUsed4Nesting()) {
			std::cerr << RED
				  << "WindFlow Error: Win_MapReduce_GPU has already been used in a nested structure"
				  << DEFAULT_COLOR << std::endl;
			exit(EXIT_FAILURE);
		}
		else {
		...
		// check the compatibility of the windowing/batching parameters
		if (_wm.win_len != _win_len || _wm.slide_len != _slide_len
		    || _wm.triggering_delay != _triggering_delay
		    || _wm.winType != _winType || _wm.batch_len != _batch_len
		    || _wm.n_thread_block != _n_thread_block) {
			std::cerr << RED
				  << "WindFlow Error: incompatible windowing and batching parameters between Key_Farm_GPU and Win_MapReduce_GPU"
				  << DEFAULT_COLOR << std::endl;
			exit(EXIT_FAILURE);
		}
